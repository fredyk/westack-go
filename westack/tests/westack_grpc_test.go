...
func testSpecialDatePlaceholder(t *testing.T, specialDateKey string, specialDatePlaceholder string) (wst.A, error) {
	// Encode uri components using the same encoding as the browser
	encodedKey := encodeUriComponent(specialDateKey)
	encodedPlaceholder := encodeUriComponent(specialDatePlaceholder)
	// use fmt
	endpointWithFilter := fmt.Sprintf("/notes?filter={\"where\":{\"created\":{\"%s\":\"%s\"}}}", encodedKey, encodedPlaceholder)
	fmt.Printf("Sending query: %s\n", endpointWithFilter)
	out, err := invokeApiA(t, "GET", endpointWithFilter, nil, nil)
	assert.NoError(t, err)
	return out, err
}
...
func TestMain(m *testing.M) {
	...
	plainUser := wst.M{
		"username": fmt.Sprintf("user_%d", createRandomInt()),
		"password": "abcd1234.",
	}
