...
func Test_InvalidCasbinOutputDirectory1(t *testing.T) {
	...
	app.Viper.Set("casbin.policies.outputDirectory", "/home/invalid/a")

	// recover from panic
	defer func() {
		if r := recover(); r != nil {
			assert.Regexp(t, "Error while loading models: could not (check|create) policies directory /home/invalid/a: (stat|mkdir) /home/invalid/a: permission denied", r)
		} else {
