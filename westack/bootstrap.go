...
func (app *WeStack) setupModel(loadedModel *model.Model, dataSource *datasource.Datasource) {
	...
	casbinModel, err, adapter := createCasbinModel(loadedModel, app, config)
	if err != nil {
		app.logger.Fatalf("failed to create casbin model: %v", err)
	}

	loadedModel.CasbinModel = &casbinModel
	loadedModel.CasbinAdapter = &adapter

	err = adapter.SavePolicy(casbinModel)
	if err != nil {
		app.logger.Fatalf("failed to save casbin policies: %v", err)
	}
